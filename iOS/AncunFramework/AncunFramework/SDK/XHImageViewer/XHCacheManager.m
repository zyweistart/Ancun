...
- (NSString *)_cacheDirectory {
    if(_cacheDirectoryPath == nil){
        ...
        _cacheDirectoryPath = [paths.lastObject stringByAppendingPathComponent:self.identifier.md5];
...
- (void)removeCacheForURL:(NSURL *)url {
    if(url.absoluteString.length>0){
        [self _removeCacheForHash:url.absoluteString.md5];
    }
}
...
- (void)storeData:(NSData *)data forURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    if(data && url.absoluteString.length > 0){
        [self _storeData:data forHash:url.absoluteString.md5 storeMemoryCache:storeMemoryCache];
    }
}
...
- (NSData *)localCachedDataWithURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        return [self _localCachedDataWithHash:url.absoluteString.md5];
    }
    return nil;
}
...
- (NSData *)dataWithURL:(NSURL *)url storeMemoryCache:(BOOL)storeMemoryCache {
    ...
    return [self _cachedDataWithHash:url.absoluteString.md5 storeMemoryCache:storeMemoryCache];
}

- (BOOL)existsDataForURL:(NSURL *)url {
    if(url.absoluteString.length > 0){
        NSString *path = [self _pathForHash:url.absoluteString.md5];
...
- (void)storeMemoryCacheWithImage:(UIImage *)image forURL:(NSURL *)url {
    if(image && url.absoluteString.length > 0){
        [self storeMemoryCacheWithImage:image forHash:url.absoluteString.md5];
    }
}
