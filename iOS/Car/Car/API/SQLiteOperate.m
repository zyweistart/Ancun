...
- (NSMutableArray*)getAllVideoItem
{
    ...
    for(NSDictionary *d in lists){
        VideoItem *msg=[[VideoItem alloc]init];
...
{
    NSString *SQL=[NSString stringWithFormat:@"INSERT INTO MESSAGENOTIFICATION (TITLE,CONTEXT,OPERATOR,URL,TYPE,DATE,READ) VALUES ('%@','%@','%@','%@','%@','%@','0')",item.title,item.content,item.oper,item.url,item.type,item.date];
    return [self execSql:SQL];
}

- (BOOL)updateVideoItem:(VideoItem*)item
{
    NSString *SQL=[NSString stringWithFormat:@"UPDATE MESSAGENOTIFICATION SET READ = '1' WHERE ID = %@",item.key];
    return [self execSql:SQL];
}

- (BOOL)deleteVideoItem:(VideoItem*)item
{
    NSString *SQL=[NSString stringWithFormat:@"DELETE FROM MESSAGENOTIFICATION WHERE ID = %@",item.key];
    return [self execSql:SQL];
}
